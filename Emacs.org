...
- Remember to set =(org-appear-mode -1)= when using =org-present=
  - [[https://github.com/daviwil/dotfiles/blob/master/Emacs.org#org-present][Daviwil config]]
- Cape
- Figure out why org appear isn't working
- Setup magit
  - Remember to use =:commands magit-status= to defer
...
- Forge and Magit(evil-collection)
- Check out Jake-B's hydra [[https://github.com/jakebox/jake-emacs#hydra][config]]
- Haji-ali/progress for modeline progress (tex-compile)
...
#+begin_src emacs-lisp
  ...
  (defvar reiki/frame-transparency '(90 . 90))
...
#+begin_src emacs-lisp
  ...
  ;; Sets transparency
  ;; (set-frame-parameter (selected-frame) 'alpha reiki/frame-transparency)
  ;; (add-to-list 'default-frame-alist '(alpha . reiki/frame-transparency))
...
PDF-Tools displays in very high quality and renders much faster than DocView in my experience. I do recall random bugs of LaTeX live preview not updating sometimes with PDF-Tools, I forgot how one is supposed to fix it as it was inconsistent. PDF-Tools as the name implies works mostly with PDF's only so DocView has a leg up if you plan on using DVI or PS files. PDF-Tools does not have Evil keybinds by default but I've seen people rebind the keys to make it functionally similar. (I'd link you to a page containing the config but this comment would probably be deleted. Just trust me it exists.)
...
#+begin_src emacs-lisp
  ...
  (use-package latex-preview-pane
    :mode ("\\.tex\\'" . latex-preview-pane-mode)
  ;;(latex-preview-pane-enable) ; Uncomment to load latex-preview-pane automatically
    )
